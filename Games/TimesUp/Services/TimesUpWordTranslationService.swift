//
//  TimesUpWordTranslationService.swift
//  TimesUp
//
...
@MainActor
class TimesUpWordTranslationService: ObservableObject {
    ...
    private func checkAIAvailability() {
        ...
        if #available(iOS 26.0, *) {
            ...
            case .available:
                ...
                print("🤖 TimesUpWordTranslationService: Apple Intelligence verfügbar")
            case .unavailable:
                isAIAvailable = false
                print("⚠️ TimesUpWordTranslationService: Apple Intelligence nicht verfügbar – Verwende Fallback-Wörterbuch")
            }
            return
        }
        #endif
        isAIAvailable = false
        print("⚠️ TimesUpWordTranslationService: Apple Intelligence nicht verfügbar – iOS-Version zu alt")
    }
    ...
    func translateToEnglish(_ term: String) async -> String {
        ...
        if #available(iOS 26.0, *), isAIAvailable, let session = session as? LanguageModelSession {
            ...
            do {
                ...
                print("⚠️ TimesUpWordTranslationService: Übersetzung fehlgeschlagen -> \(error.localizedDescription)")
            }
        }
        #endif
