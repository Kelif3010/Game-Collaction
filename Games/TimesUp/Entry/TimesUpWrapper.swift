...
struct TimesUpWrapper: View {
    ...
    @StateObject private var categoryViewModel = TimesUpCategoryViewModel()
    ...
    var body: some View {
        ...
        SettingsView(categoryViewModel: categoryViewModel)
            // WICHTIG: Die Spracheinstellung muss hier injected werden
            .environment(\.locale, activeLanguage.locale)
            .onChange(of: useSystemLanguage) { _, _ in
                categoryViewModel.updateLanguage(activeLanguage)
            }
