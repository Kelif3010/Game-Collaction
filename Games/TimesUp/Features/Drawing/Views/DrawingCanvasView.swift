...
struct DrawingCanvasView: View {
        ...
        GeometryReader { geometry in
            ZStack {
                ...
                Rectangle()
                    ...
                    .onAppear {
                        drawingViewModel.canvasSize = geometry.size
                    }
                    .onTapGesture { location in
                        // Tap-Geste für Fill-Tool
                        if drawingViewModel.selectedTool == .fill {
                            print("🪣 DEBUG: Fill tap at: \(location)")
                            drawingViewModel.fillArea(at: location, with: drawingViewModel.selectedColor)
                        } else {
                            // Normaler Punkt für andere Tools
                            drawingViewModel.startStroke(at: location)
                            drawingViewModel.addPoint(to: location)
                            drawingViewModel.finishStroke()
                        }
                    }
                    .gesture(
                ...
                Canvas { context, size in
                    ...
                    for stroke in drawingViewModel.strokes {
                        renderStroke(stroke, context: context)
                    }
                    
                    // Render current stroke being drawn
                    if let currentStroke = drawingViewModel.currentStroke {
    ...
    private func handleDragChanged(_ value: DragGesture.Value, in canvasSize: CGSize) {
        ...
        if drawingViewModel.currentStroke == nil {
            drawingViewModel.startStroke(at: point)
        } else {
            drawingViewModel.addPoint(to: point)
        }
    }
