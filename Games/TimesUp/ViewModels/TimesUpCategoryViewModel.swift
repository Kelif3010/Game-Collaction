...
@MainActor
class TimesUpCategoryViewModel: ObservableObject {
    ...
    // DEBUG logging
    private let loggerPrefix = "🧩 TimesUpCategoryViewModel"
    ...
    private let aiGenerator = TimesUpAICategoryService()
    ...
    /// Uses the external TimesUpDefaultCategories struct to keep this file clean.
    private func loadDefaultCategories(for language: AppLanguage = .german) {
        ...
        categories = TimesUpDefaultCategories.getData(for: language)
    ...
    private func applyDefaultLanguage(_ language: AppLanguage) {
        let defaults = TimesUpDefaultCategories.getData(for: language)
